  ...
  return (
    <div className="searchable-list">
      ...
      <div className="searchable-list-items">
        <button type="button" className={selected === "all" ? "list-item active" : "list-item"} onClick={() => handleSelect("all")}>{allLabel}</button>
        {filtered.map((item) => (
          <button key={item} type="button" className={selected === item ? "list-item active" : "list-item"} onClick={() => handleSelect(item)}>{item}</button>
        ))}
      </div>
    </div>
  );
